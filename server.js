...
app.get("/locations/:id", (req, res) => {
  ...
  const location = data.locations.find((loc) => loc.id === id); // Find the location with the matching id

  if (location) {
    res.json(location); // If a location with the specified id is found, return it as JSON
  } else {
    res.status(404).json({ message: "Location not found" }); // If no matching location is found, return a 404 status and an error message
  }
});
